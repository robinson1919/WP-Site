...
if ( ! class_exists( 'Astra_Portfolio_Admin' ) ) :
	...
	class Astra_Portfolio_Admin {
		...
		function meta_boxe_callback( $post ) {
			...
			<table class="widefat astra-portfolio-table">
				<tr class="astra-portfolio-row">
					<td class="astra-portfolio-heading"><?php _e( 'Portfolio Type', 'astra-portfolio' ); ?></td>
					<td class="astra-portfolio-content">
						<?php
						echo self::get_portfolio_type_label( $portfolio_type );
						?>
					</td>
				</tr>
				<tr class="astra-portfolio-row">
					<td class="astra-portfolio-heading"><?php _e( 'Thumbnail Image', 'astra-portfolio' ); ?></td>
					<td class="astra-portfolio-content">
				...
				switch ( $portfolio_type ) {
					...
					case 'iframe':
						?>
									<tr class="astra-portfolio-row">
										<td class="astra-portfolio-heading"><?php _e( 'Enter URL', 'astra-portfolio' ); ?></td>
									...
									<tr class="astra-portfolio-row">
										<td class="astra-portfolio-heading"><?php _e( 'Open in New Tab', 'astra-portfolio' ); ?></td>
										<td class="astra-portfolio-content">
											<input type="checkbox" <?php checked( $open_in_new_tab, 1 ); ?> value="1" class="astra-input-text" name="astra-site-open-in-new-tab" />
										</td>
									</tr>
									<tr class="astra-portfolio-row">
										<td class="astra-portfolio-heading"><?php _e( 'Add Call-to-action', 'astra-portfolio' ); ?></td>
										<td class="astra-portfolio-content">
											<textarea class="astra-input-text" rows="4" name="astra-site-call-to-action"><?php echo $call_to_action; ?></textarea>
											<p class="description"><?php _e( 'This allows you to add a call-to-action on the preview bar.', 'astra-portfolio' ); ?></p>
										</td>
									</tr>
									<?php
						break;
					...
					case 'image':
						?>
									<tr class="astra-portfolio-row">
										<td class="astra-portfolio-heading"><?php _e( 'Portfolio Image', 'astra-portfolio' ); ?></td>
										<td class="astra-portfolio-content">
											<div class="astra-portfolio-image">
					...
					case 'video':
						?>
									<tr class="astra-portfolio-row">
										<td class="astra-portfolio-heading"><?php _e( 'Video URL', 'astra-portfolio' ); ?></td>
										<td class="astra-portfolio-content">
											<div class="astra-portfolio-image">
					...
					case 'page':
						?>
									<tr class="astra-portfolio-row">
										<td class="astra-portfolio-heading"><?php _e( 'Open Portfolio Item in', 'astra-portfolio' ); ?></td>
										<td class="astra-portfolio-content">
											<select name="astra-site-open-portfolio-in">
												<option value="new-tab" <?php selected( $open_portfolio_in, 'new-tab' ); ?> /><?php _e( 'New Tab', 'astra-portfolio' ); ?></option>
												<option value="same-tab" <?php selected( $open_portfolio_in, 'same-tab' ); ?> /><?php _e( 'Same Tab', 'astra-portfolio' ); ?></option>
												<option value="iframe" <?php selected( $open_portfolio_in, 'iframe' ); ?> /><?php _e( 'iFrame', 'astra-portfolio' ); ?></option>
											</select>
											<p class="description"><?php _e( 'Select where you wish to show a portfolio item when the user clicks on it.', 'astra-portfolio' ); ?></p>
										</td>
									</tr>
									<tr class="astra-portfolio-row">
										<td class="astra-portfolio-heading"><?php _e( 'Add Call-to-action', 'astra-portfolio' ); ?></td>
		 ...
		 */
		function get_portfolio_type_label( $portfolio_type = '' ) {
			...
			if ( ! empty( $portfolio_types ) ) {
				foreach ( $portfolio_types as $key => $portfolio ) {
					if ( $portfolio_type === $portfolio['key'] ) {
						return esc_html( $portfolio['label'] );
					}
				}
			}
		 ...
		 */
		function change_portfolio_taxonomy_url_slug( $args, $taxonomy, $object_type ) {
		 ...
		 * @return array             Filtered arguments.
		 */
		function change_portfolio_url_slug( $args, $post_type ) {
		 ...
		 * @since 1.0.3
		 */
		function add_portfolio_type_class( $classes = '' ) {
			if ( 'astra-portfolio' !== get_current_screen()->id && 'edit-astra-portfolio' !== get_current_screen()->id ) {
				return $classes;
			}
		 ...
		 */
		function is_portfolio_type_excluded_from_search( $portfolio_type = '' ) {

			$portfolio_types = Astra_Portfolio_Page::get_instance()->get_portfolio_types();
			$portfolio_slugs = wp_list_pluck( $portfolio_types, 'key' );

			// Remove the `page` type.
			$key = array_search( 'page', $portfolio_slugs, false );
			if ( false !== $key ) {
				unset( $portfolio_slugs[ $key ] );
			}

			if ( in_array( $portfolio_type, $portfolio_slugs ) ) {
				return true;
			}
