	...
	class Astra_Portfolio_Import_Image {
		...
		private function get_saved_image( $attachment ) {
			...
			if ( apply_filters( 'astra_portfolio_image_importer_skip_image', false, $attachment ) ) {
				error_log( 'Failed! Avoided by filter \'astra_portfolio_image_importer_skip_image\' ' . $attachment['url'] );
				return $attachment;
			}
			...
			if ( isset( $this->already_imported_ids[ $attachment['id'] ] ) ) {
				error_log( 'Failed! Already imported? Then return! - ' . $attachment['url'] );
		...
		public function import( $attachment ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				error_log( sprintf( __( 'Failed! WP Error - %1$s %2$s', 'astra-portfolio' ), $response->get_error_message(), $attachment['url'] ) );
				return $attachment;
			} elseif ( 200 !== $response_code ) {
				/* translators: %1$s is error response code and %2$s is image URL */
				error_log( sprintf( __( 'Failed! Invalid Response Code %1$s! Expected response code 200 - %2$s', 'astra-portfolio' ), $response_code, $attachment['url'] ) );
				return $attachment;
			} elseif ( empty( $response_body ) ) {
				error_log( __( 'Failed! Empty image content.', 'astra-portfolio' ) );
				return $attachment;
			}
			...
			if ( isset( $upload['error'] ) && ! empty( $upload['error'] ) ) {
				/* translators: %1$s is error message and %2$s is image URL */
				error_log( sprintf( __( 'Failed! WP Error - %1$s %2$s', 'astra-portfolio' ), $upload['error'], $attachment['url'] ) );
				return $attachment;
			}
